= ⚇ ddot.it and RDF
:page-page-title: Resource Description Framework (RDF)
:page-parent: Developer Guide
:page-nav_order: 10
:toc:

Resource Description Framework (RDF) is also using triples, but RDF has more features – and is more complex
 then ddot.it.
RDF is not even a syntax, but a datamodel with several syntaxes.
ddot.it compares probably best to https://en.wikipedia.org/wiki/Turtle_(syntax)[Turtle].
...
== Summary
ddot.it does not replace RDF.
It is a simpler syntax, with fewer features, that is easier to type.
