...
* *Readers now scan every file by default.* Previously the default was "all
 ...
 optional _exclude non-marked files_ mode.
...
`
 ...
 region begins on the *next* line. The command _name_ is still matched
 exactly, so `!!office` is not `!!off`.

CAUTION: any line containing the command triggers it — including prose
 _about_ the command. A document that discusses ddot.it syntax should keep such
 mentions inside a https://ddot.it/block[`ddot.it/block`], where commands are
 inert, or accept that the rest of the file is excluded.

* *A command requires the slash and a name.* Bare `ddot.it` is a _document
 marker_, not a command. The four spellings `https://ddot.it/NAME`,
...
* *Whitespace is Tab plus every Unicode space separator.* `WS` was space and tab; it is now
 ...
 the rest count as whitespace wherever a space does. These characters are _visually
 indistinguishable_ from a space and are inserted routinely by word processors, wikis, PDF
 copy-paste and macOS Option+Space; treating them as ordinary text silently produced a
 _different_ node that looked identical to the intended one:
 `
...
`
 ...
 over-strips U+FEFF, and Java's `String.strip` _excludes_ U+00A0 and U+202F. See the
 link:spec/ddot-it-parse.html#whitespace[Parse Specification].
...
`
 ...
 See the link:spec/ddot-it-parse.html[Parse Specification] for the exact rule.
...
* *A `,,` part holds _entries_, and pairs and free-text notes mix freely.*
...
* *Adjacent note lines in a `,,` block are one note*, joined by newlines, so a
 ...
 after it starts a new note. Two _adjacent_ notes are therefore not writable in
...
+
attaches `year = 2123` to the `john ..address.. …` triple.
Previously such a line was read as a *new* triple whose subject was the literal`,,`or`;;`.

* *A continuation line may itself open a block*, so one logical line can carry
 several blocks — for example a block subject _and_ a block object:
+
[source,ddot]
----
!!block
...
* *Dropped the planned `ddot.it/verbatim` command.*
 https://ddot.it/block[`ddot.it/block`] already takes its body verbatim —
...
* *`property` and `relation` are types.* A relation can now be classified with
 ...
 the _thing_ / _value_ distinction that ddot.it's single string datatype cannot
 express. See the link:spec/ddot-it-vocabulary.html[Vocabulary Specification].
...
* *The triple event format has one home and one key vocabulary.* The wire format is now
 ...
 link:spec/ddot-it-parse.html#events[Parse Specification] — fields, meta-pair shape, key order and
...
* *The grammar is written in the EBNF of https://www.w3.org/TR/xml/#sec-notation["XML 1.0, section 6"].*
...
* *`ddot.it/label` has no page of its own.* `label` is an ordinary relation, so it is
 ...
 link:spec/ddot-it-vocabulary.html#label[Vocabulary Specification]; `https://ddot.it/label` no
 longer resolves.
...
* *`ddot.it/block` fills four positions, not three.* Subject, object, meta-object *and* the
 ...
 other three. It is still _not_ an opener in a relation or a meta-relation, which hold names.
...
* Added https://ddot.it/block[`ddot.it/block`] command
* New `has content` relation
* Added custom element syntax (`<ddot-it>`) to link:reader/reader-html.html[HTML reader]
...
* Initial https://ddot.it/rel[relations list]
